...
function getPhilosophersForPosters() {
  const philosopherArray = [];
  philosophers.results.forEach((philosopher) => {
    philosopherArray.push(philosopher.name);
  });

  return shuffle(philosopherArray);
}

function getIdeasArray() {
  const ideasArray = [];

  philosophers.results.forEach((philosopher) => {
    philosopher.ideas
      .map((idea) => ({
        imgUrl: philosopher.photo,
  ...
  return shuffle(ideasArray);
}

function getIdeasObject() {
  const ideasObject = {};
  getIdeasArray().forEach((idea, index) => {
    ideasObject[index] = { id: index, ...idea };
  });
...
function getSchoolsArray() {
  const schoolsArray = [];
  philosophers.results.forEach((philosopher) => {
    philosopher.school.forEach((school) => {
      if (schoolsArray.indexOf(school) === -1) {
        schoolsArray.push(school);
      }
    });
  });
  return shuffle(schoolsArray);
}

function getSchoolsWithPhilosophers() {
  const schoolsWithPhilosophers = {};

  philosophers.results.forEach((philosopher) => {
    philosopher.school.forEach((school) => {
      if (!schoolsWithPhilosophers[school]) {
        schoolsWithPhilosophers[school] = [];
      }
      schoolsWithPhilosophers[school].push(philosopher.name);
    });
  });
...
function getRandomIdea() {
  const ideasArray = getIdeasArray();
...
function getPhilosopherByName(philosopherName) {
  const philosopher = philosophers.results.find((philosopher) => {
    return philosopher.name === philosopherName;
  });

  return philosopher;
}

function getPhilosopherBySchoolName(schoolName) {
  return getSchoolsWithPhilosophers()[schoolName];
}

export {
  getIdeasArray,
  getPhilosophersForPosters,
  getSchoolsArray,
  getRandomIdea,
  getPhilosopherByName,
  getPhilosopherBySchoolName,
